...
install_packages() {
    echo -e "${ORANGE}$1${NC}"
    sudo luet install -y "${@:2}" || handle_error "Failed to install: $1"
...
install_packages "Installing ZSH / FISH shells and Plug-ins""${software_packages[@]}"
	...
	# Install some fonts
sudo luet install -y fira fontawesome firacode
...
#    sudo luet update
#    sudo luet install -y linux-firmware xorg-driver-video-amdgpu
    echo "AMD GPU firmware installed successfully."

else
    # Install video acceleration for HD Intel i965
#    sudo luet update
#    sudo luet install xorg-driver-video-intel
#    sudo luet install -y libva-intel-driver
