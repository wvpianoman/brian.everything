...
essential_packages=(
    acl aria2 attr autoconf automake bash-completion bc binutils btop busybox ca-certificates cifs-utils libcjson1 codec2 cookietool cowsay cron curl gir1.2-dbusglib-1.0 dconf-editor dialog direnv dnsutils duf earlyoom easyeffects espeak espeak-ng fancontrol mbpfan fd-find figlet firmware-realtek flatpak fortune-mod fortunes fortunes-min gdebi git gnupg2 grep grub-customizer gtk2-engines haveged ibus-gtk4 iptables jq ublock-origin-doc webext-ublock-origin-firefox mpg123 snmpd net-tools nftables openssh-{client,server} ostree p7zip p7zip-full p7zip-rar packagekit pandoc pip pipewire-{audio,doc} pkg-config plocate powertop python3 python3-pip python3-setproctitle qrencode ripgrep rsync rygel sassc screen socat sshpass sxiv tar terminator thefuck tlp tlp-rdw tlpui tumbler tumbler-plugins-extra ufw ugrep un{zip,rar} unrar-free webext-ublock-origin-chromium wget wget2 wsdd xclip zip systemd-zram-generator zramswap-sysvinit-compat zram-tools zstd yad
)
# kde packages
kde_packages=(
    akonadi-import-wizard dolphin-plugins ffmpeg ffmpegthumbnailer ffmpegthumbs kate kdegraphics-thumbnailers kdepim-addons yakuake korganizer plasma-discover-backend-{flatpak,fwupd} plasma-firewall kate 
)
...
# software packages
software_packages=(
    blender blender-data ghostwriter gimp gimp-help-en krita inkscape boomaga digikam flameshot rclone rclone-browser rhythmbox scribus scribus-doc scribus-template shotwell simplescreenrecorder syncthing syncthing-gtk telegram-desktop uget vlc yakuake nheko nano fastfetch neovim neovim-qt variety meld #virt-manager
)
