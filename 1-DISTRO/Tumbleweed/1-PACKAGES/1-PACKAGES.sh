...
install_packages() {
    echo -e "${ORANGE}$1${NC}"
    sudo zypper5 install -y  "${@:2}" || handle_error "Failed to install: $1"
...
# Install Megasync
wget https://mega.nz/linux/repo/openSUSE_Tumbleweed/x86_64/megasync-openSUSE_Tumbleweed.x86_64.rpm && sudo zypper in "$PWD/megasync-openSUSE_Tumbleweed.x86_64.rpm"
...
sudo zypper install -y curl cabextract xorg-x11-font-utils fontconfig -y
	sudo zypper install -y fontawesome-fonts powerline-fonts 'google-roboto*' 'mozilla-fira*' fira-code-fonts meslo-nerd-fonts firacode-nerd-fonts
	sudo zypper install -y redhat-{mono,text,display}-{fonts,vf-fonts} xorg-x11-fonts-ISO8859-1-100dpi google-noto-emoji-color-fonts droidsansmono-nerd-fonts
...
-----------------------------------------------
	...
	# sudo zypper remove profile-sync-daemon
...
#atuin sync
	...
	comm -23 <(sudo zypper repoquery --installonly --latest-limit=-1 -q | sort) <(sudo zypper list installed | awk '{print $1}' | sort) >/tmp/orphaned-pkgs

	if [ -s /tmp/orphaned-pkgs ]; then
		sudo zypper remove $(cat /tmp/orphaned-pkgs) -y --skip-broken
	else
